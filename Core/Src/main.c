...
/* USER CODE BEGIN 0 */
	...
	//Circular DMA continuously transfers the converted values into memory without stopping,
	//continuously updating the buffer with the latest ADC data, making it ideal for
	//applications that require real-time and continuous data acquisition. This approach
	//reduces CPU overhead since the DMA automatically restarts after completing a sequence.
	//if the CPU doesn’t read ADC data in time, some data may be lost

	//configuration:
	//Scan Conversion Mode enabled, cause multichannel
	//Continuous Conversion Mode enabled, st adc keeps converting the analog signal automatically
	//DMA Continuous Request enabled, st the DMA can automatically move the ADC data automatically after conv done
	...
	//set number of conversions to num of channels used
...
{
	isADCFinished = 1;
	//we should process the data here
}
